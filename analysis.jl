...
function get_matrix(ds, idx)
	arr = []
	for d in ds
		push!(arr, getindex.(d["data"], idx))
	end
	return reverse(reduce(hcat, arr), dims = 1)
end

# ╔═╡ 9c30bc3f-907f-4214-aeb9-abbba645ce4f
begin
	ndim = get_matrix(ds, 6) ./ N^2
	Mππ = get_matrix(ds, 3) ./ N^2
	Mπ0 = (get_matrix(ds, 4) .+ get_matrix(ds, 5)) ./ N^2
end;
